...
def evaluate(raw_request_jcs: str, strategy: Strategy) -> str:
    ...
    schedule_rows = cast(list[object], schedule)
    ledger_rows = cast(list[object], ledger)
    for row_value in schedule_rows:
        if not isinstance(row_value, dict):
            raise ValueError("v1 schedule row is invalid")
        row = cast(dict[str, object], row_value)
        row["fee"] = fee.as_string
        row["payment"] = _post(_fraction(row["payment"]) + Fraction(fee.amount))
    for row_value in ledger_rows:
        ...
        if type(month) is not int:
            continue
        posted_months = min(max(month, 0), len(schedule_rows))
        accumulated_fee = Fraction(fee.amount) * posted_months
        if 1 <= month <= len(schedule_rows):
            cost = _fraction(row["nonrecoverable_housing_cost"]) + Fraction(fee.amount)
            row["nonrecoverable_housing_cost"] = _post(cost)
        row["cash"] = _post(_fraction(row["cash"]) - accumulated_fee)
        row["cumulative_housing_cost"] = _post(
            _fraction(row["cumulative_housing_cost"]) + accumulated_fee
        )
        row["liquidity"] = row["cash"]
        row["net_worth"] = _post(_fraction(row["net_worth"]) - accumulated_fee)
    normalized_value = base["normalized_input"]
    if not isinstance(normalized_value, dict):
        raise ValueError("v1 normalized input is invalid")
    normalized_input = cast(dict[str, object], normalized_value)
...
def validate_outcome(outcome_jcs: object) -> None:
    ...
    if set(normalized_map) != {"cash_down_payment", "comparison_opening_cash", "effective_monthly_rate", "fee_amount", "principal", "property_price", "term_months"}:
        raise ValueError("sealed normalized input is invalid")
    if not isinstance(trace, dict):
        raise ValueError("sealed trace is invalid")
    trace_map = cast(dict[str, object], trace)
    if set(trace_map) != {"comparison_ledger", "contractual_schedule"}:
        raise ValueError("sealed trace is invalid")
    schedule = trace_map["contractual_schedule"]
    ledger = trace_map["comparison_ledger"]
    if not isinstance(schedule, list) or not isinstance(ledger, list):
        raise ValueError("sealed trace rows are invalid")
    for row_value in cast(list[object], schedule):
        if not isinstance(row_value, dict):
            raise ValueError("sealed schedule row is invalid")
        row = cast(dict[str, object], row_value)
        if set(row) != {"amortization", "closing_principal_balance", "fee", "interest", "month", "opening_principal_balance", "payment"}:
            raise ValueError("sealed schedule row is invalid")
    ...
    for row_value in cast(list[object], ledger):
        if not isinstance(row_value, dict):
...
def _fraction(value: object) -> Fraction:
    if not isinstance(value, str):
        raise ValueError("v1 monetary trace value is invalid")
